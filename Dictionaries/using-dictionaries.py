...
"""
From within the using-dictionaries.py file, we'll be modifying the users list so that the assertions throughout the file all succeed eventually. 
As we're working through the tasks we can run the file. Whenever Python gets to a line that starts with assert, 
it will raise an error and stop executing if we haven't met the criteria. The first error will look like this:

$ python3.7 using-dictionaries.py
Traceback (most recent call last):
  File "using-dictionaries.py", line 3, in <module>
...
emails.update({'ashley':'ashley@example.com', 'craig':'craig@example.com', 'elizabeth':'elizabeth@example.com'})
print(emails)
...
emails.update({'dalton':'dalton@example.com'})
